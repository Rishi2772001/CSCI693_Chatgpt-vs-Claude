...
def run_test_and_measure(func, n, expected):
    """
    Runs a single test: func(n)
    Measures wall-clock time, CPU time, and peak memory.
    Returns a dictionary with the result, correctness, and performance metrics.
    """
...
def test_solution(solution_func):
    ...
    for n, expected in TEST_CASES:
        metrics = run_test_and_measure(solution_func, n, expected)
        results.append(metrics)
    return results

def generate_learning_curve_complexity(solution_func, sizes, num_runs=3):
    """
    For each input size in sizes, runs solution_func(n) num_runs times
    to get average execution times, then uses linear regression on log-log scale
    to estimate the exponent.
    Returns:
      sizes: list of input sizes,
      avg_times: corresponding average times,
      exponent: estimated complexity exponent.
    """
    avg_times = []
    for size in sizes:
        run_times = []
    ...
    # Fit linear regression on log10(sizes) vs log10(avg_times)
...
def main():
    # Import solution modules and get the climbStairs functions.
    chatgpt_mod = importlib.import_module(CHATGPT_MODULE_NAME)
    claude_mod  = importlib.import_module(CLAUDE_MODULE_NAME)
    
    if not hasattr(chatgpt_mod, "climbStairs") or not hasattr(claude_mod, "climbStairs"):
        raise ImportError("Both modules must define a climbStairs(n) function.")
    
    chatgpt_func = chatgpt_mod.climbStairs
    claude_func  = claude_mod.climbStairs
    ...
    for i, (testcase, cg_metrics, cl_metrics) in enumerate(zip(TEST_CASES, chatgpt_results, claude_results), start=1):
        n, expected = testcase
        print(f"Test #{i}: n={n}, expected={expected}")
    ...
    # Generate learning curves for both solutions.
    plt.figure(figsize=(8,6))
    ...
    plt.xlabel("Input size (n)")
    plt.ylabel("Average execution time (s)")
    plt.title("Learning Curve Comparison for climbStairs")
    plt.xscale("log")
    plt.yscale("log")
    plt.grid(True, which="both", linestyle="--")
    plt.legend()
    ...
    # Optionally, print the estimated exponents.
