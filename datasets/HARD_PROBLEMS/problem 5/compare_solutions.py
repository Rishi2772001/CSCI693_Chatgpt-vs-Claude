...
##############################################
# Module settings for Wildcard Pattern Matching (Problem 5)
##############################################
...
##############################################
# Learning Curve Generation for Wildcard Pattern Matching
##############################################
...
def generate_random_pattern(s, wildcard_prob=0.1, star_prob=0.1):
    """
    Generates a pattern based on string s such that it still matches s.
    For each character in s, with probability wildcard_prob, replace it with '?'.
    Also, with probability star_prob, insert a '*' before the character.
    """
    pattern = []
    for ch in s:
...
def generate_learning_curve_complexity(func, sizes, num_runs=3, wildcard_prob=0.1, star_prob=0.1):
    """
    For each string length L in sizes, generates a random string s and a pattern p (derived from s)
    such that p matches s. Runs func(s, p) num_runs times, computes the average execution time,
    and applies log–log regression to estimate the complexity exponent.
...
def plot_learning_curves(sizes1, times1, exp1, sizes2, times2, exp2, filename="learning_curve_wildcard.png"):
    ...
    plt.title("Learning Curve Comparison for Wildcard Pattern Matching")
    plt.xscale("log")
    plt.yscale("log")
...
##############################################
# Main Comparison for Wildcard Pattern Matching
##############################################
def main():
    ...
    # Determine raw decision based on wall time and memory only.
    ...
    if not (chatgpt_all_correct and claude_all_correct):
        ...
        # Compute learning curve exponents.
        ...
        # Use a threshold for comparing the exponents.
    ...
    sizes_chat, times_chat, exp_chat_wc = generate_learning_curve_complexity(chatgpt_func, sizes, num_runs)
    sizes_claude, times_claude, exp_claude_wc = generate_learning_curve_complexity(claude_func, sizes, num_runs)
    
    plt.figure(figsize=(8,6))
    plt.plot(sizes_chat, times_chat, marker='o', label=f"ChatGPT (exp ≈ {exp_chat_wc:.2f})")
    plt.plot(sizes_claude, times_claude, marker='s', label=f"Claude (exp ≈ {exp_claude_wc:.2f})")
    plt.xlabel("String length (L)")
    plt.ylabel("Average execution time (s)")
    plt.title("Learning Curve Comparison for Wildcard Pattern Matching")
    plt.xscale("log")
    ...
    plt.grid(True, which="both", linestyle="--")
    plt.legend()
    learning_curve_file = os.path.join(os.path.dirname(__file__), "learning_curve_wildcard.png")
    plt.savefig(learning_curve_file)
    plt.close()
    print(f"Learning curve for Wildcard Pattern Matching saved to {learning_curve_file}")
